...
//flip 0
      if (should_do == "do without Flip") {
        PlaceDB::Configuration temp = gigaplace::Operator::creatTempConfig(pl_db, config_list.at(i), config_list.at(j));
...
void gigaplace::Operator::pairSingleMos(gigaplace::PlaceDB &pl_db, std::vector<index> &single_mos_ids,
                                        std::vector<PlaceDB::Configuration> &pair_list) {
    bool pair_flag = false;
    for (auto &single_mos_idx: single_mos_ids) {
